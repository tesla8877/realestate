...
class AddPropertyViewModel (
    ...
    override fun resultToViewState(result: LoadingContentError<AddPropertyResult>) {
    ...
    private fun resultToViewEffect(result: LoadingContentError<AddPropertyResult>){
        if(result is LoadingContentError.Content){
            currentViewState.copy(
                    isADraft = false,
                    listAgents = null,
                    errors = null,
    ...
    private fun emitResultPictureModification(){
        val result: LoadingContentError<AddPropertyResult> = LoadingContentError.Content(AddPropertyResult.PictureResult(pictures))
    ...
    private fun receivePropertyData(type: String, price: Double?,
                                    ...
                                    amenities: List<TypeFacility>, contextApp: Context
    ){
    ...
    private fun checkLocationAndMap(geoApi: GeoApiResponse){
        val results = geoApi.results
        if(results.isNotEmpty()){
            configureAddressComponent(geoApi)
    ...
    private fun configureAddressComponent(geoApi: GeoApiResponse){
        val results = geoApi.results
    ...
    private fun emitResultAddPropertyToView(){
        ...
        fun emitResult(){
            val result: LoadingContentError<AddPropertyResult> = if(errorOnPropertyCreation.isEmpty()) {
                ...
                LoadingContentError.Error(AddPropertyResult.PropertyAddedToDBResult(errorOnPropertyCreation))
            }
    ...
    private fun emitErrorFromInputs(){
        val result: LoadingContentError<AddPropertyResult> = LoadingContentError.Error(AddPropertyResult.PropertyAddedToDBResult(listErrorInputs))
        resultToViewState(result)
    }
    ...
    private fun savePropertyForLater(
            ...
            neighborhood: String, onMarketSince: String,
            isSold: Boolean, sellOn: String?,
            amenities: List<TypeFacility>
    ){
        resultToViewState(LoadingContentError.Loading())

        val tempProperty = SubProperty(
