...
class SaveDataRepository (context: Context){
    ...
    var subProperty: SubProperty? = null
        get(){
            if(field == null) {
                val gson = Gson()
                val json = sharedPreferences.getString(KEY_PREF_TEMP_PROPERTY, null)
                field = gson.fromJson(json, SubProperty::class.java)
            }
            return field
        }
        set(value){
    ...
    fun saveModifiedProperty(modifiedProperty: SubProperty?, idProperty: String){
        val gson = Gson()
    ...
    fun getSavedModifyProperty(idProperty: String?): SubProperty?{
        val gson = Gson()
        val json = sharedPreferences.getString(getKeyProperty(idProperty), null)
        return gson.fromJson(json, SubProperty::class.java)
    }
